...
const fetchGif = async (searchRequest) => {
    try {
        const response = await fetch(`https://api.giphy.com/v1/gifs/search?api_key=hpvZycW22qCjn5cRM1xtWB8NKq4dQ2My&q=${searchRequest}&limit=1`)
        ;
        if (!response.ok) {
        ...
        else {const gifUrl = gifData.data[0].images.original.url;
        ...
        gifContainer.innerHTML = '<p>Error fetching GIF. Please try again later.</p>';
    }
}
